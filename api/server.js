...
// Conectar ao banco de dados SQLite (mudar para um arquivo se necessário)
const db = new sqlite3.Database("./users.db"); // Persistente
...
// Função para verificar email e login em uso
const checkUserExists = (email, login, callback) => {
  db.get("SELECT * FROM users WHERE email = ? OR login = ?", [email, login], (err, row) => {
    if (err) {
      return callback(err);
    }
    callback(null, row);
...
app.post("/users", (req, res) => {
  ...
  checkUserExists(email, login, (err, row) => {
    if (err) {
